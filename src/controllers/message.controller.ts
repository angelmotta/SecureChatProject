...
export const getChatController = async (req: Request, res: Response) => {
    const loggedUser = req.email; // loggedUser
    if (!loggedUser) {
        res.status(400).json({ status: 0, message: 'User not authorized' });
        return;
    }
    ...
    // Request Validation
    const { error, value: payloadBody } = schemaChatRequest.validate(req.query);
    if (error) {
        console.log(error.message);
        res.status(404).json({ status: 0, message: error.message });
        return;
    ...
    const otherUser = payloadBody.email;
...
const schemaRequest = Joi.object({
    receiver: Joi.string().email({ minDomainSegments: 2 }).required().trim(),
